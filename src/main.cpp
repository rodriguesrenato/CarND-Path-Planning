    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          ego_car.SetLocalizationData(car_x,car_y,car_s,car_d,car_yaw,car_speed);
          ego_car.SetPrevPath(previous_path_x,previous_path_y,end_path_s,end_path_d);
          ...
          // sensor_fusion =  [ id, x, y, vx, vy, s, d]: The vx, vy values can
          // be useful for predicting where the cars will be in the future. For
          ...
          if (car_state == State::R) {
            ...
            vector<vector<double>> t = ego_car.GenerateTrajectory(ref_vel,lane,30,true,map_waypoints_s,map_waypoints_x,map_waypoints_y);
            next_x_vals = t[0];
            next_y_vals = t[1];
            // if (prev_size > 0) {
            //   car_s = end_path_s;
            ...
            // double target_vel = car_speed + err_vel * kp +
            //                     (err_vel - err_vel_prev) * kd -
            //                     err_safe_s_dist * 0.1;

            // std::cout << std::fixed << std::setprecision(5)
            //           << "err_vel= " << err_vel << "\tref_vel= " << ref_vel
            //           << "\tcar_speed= " << car_speed
            //           << "\ttarget_vel= " << target_vel << std::endl;
